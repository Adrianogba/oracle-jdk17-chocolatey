# Get ProductCode (GUID) of Oracle JDK 17
$jdkProductCode = (Get-WmiObject -Class Win32_Product | Where-Object { $_.Name -like "Java SE Development Kit 17*" }).IdentifyingNumber

if ($jdkProductCode) {
    # Uninstall JDK
    Write-Host "Uninstalling Oracle JDK 17..."
    Start-Process -FilePath "msiexec.exe" -ArgumentList "/x $jdkProductCode /quiet /norestart" -Wait
    Write-Host "Oracle JDK 17 uninstalled successfully."
} else {
    Write-Warning "Oracle JDK 17 not found. Skipping uninstallation."
}
...
if ($path -match ";C:\\Program Files\\Java\\jdk-17\\bin") {
    $newPath = $path -replace ";C:\\Program Files\\Java\\jdk-17\\bin", ""
